...
- Every acronym or initialised name spelled out in full with a brief descriptor on first use in the final brief. No exceptions except US, UK, UN, NATO, EU.
...
- Pass 1 and Pass 2 run as internal scaffolding. Only the network summary block from Pass 1 and the synthesis input brief from Pass 2 need to appear in the published output alongside the full Step 0 through Step 11 synthesis. The full JSON node state objects and connection registry from Pass 1, and the full threshold event and cluster JSON from Pass 2, are internal working documents and do not need to be reproduced in the published SITREP.
